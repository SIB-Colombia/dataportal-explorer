...
exports.getAllGeoOccurrenceData = function(result) {
	connection.query("select id, canonical, num_occurrences, latitude, longitude from geo_ocurrence_record_denormalized WHERE geo_ocurrence_record_denormalized.iso_country_code = 'CO'", function(err, rows, fields) {
		if (err) {
			res.send(err);
		}
		result(rows);
