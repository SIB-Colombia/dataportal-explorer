...
c.query('SELECT * FROM pointfive_cell_density order by type')
	.on('result', function(result) {
      ...
      if(counter%500 == 0) {
        console.log("Total pointfive_cell_density records extracted from MySQL database: "+counter);
			}
		})
		.on('error', function(err) {
    ...
    MongoClient.connect('mongodb://localhost/sibexplorer_dev', function(error, db) {
      ...
      rd.on('line', function(line) {
        db.collection('pointfive_cell_density').insert(JSON.parse(line), {safe: false});
        if(counter%500 == 0) {
          console.log("Total records saved in mongoDB pointfive_cell_density collection: "+counter);
        }
        counter++;
      });
